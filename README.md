...
• [📋 Потестить бот в TG](https://t.me/PersonalSmartSavingsBot) • [🚀 Быстрый старт](#-быстрый-старт) • [📋 Возможности](#-возможности) • [🏗️ Архитектура](#️-архитектура) • [📖 Документация](#-документация)
...
Опционально для алертов в Telegram:
```env
# Токен отдельного бота для алёртов и список чатов (через запятую)
TELEGRAM_BOT_ALERT=your_alert_bot_token
TELEGRAM_ALERT_CHAT_ID=123456789,-1001234567890
```
...
| `TELEGRAM_BOT_ALERT` | Токен бота для алёртов | `123456789:DEF...` |
| `TELEGRAM_ALERT_CHAT_ID` | ID чатов для алёртов (через запятую) | `123456789,-1001234567890` |
...
### 🚨 Алерты в Telegram
- В проекте подключён обработчик логов, отправляющий сообщения уровня WARNING и выше в отдельного Telegram‑бота.
- Реализация: `app/utils/alerts.py` (функция `setup_alert_logging()` вызывается при старте в `app/main.py`).
- Переменные окружения: `TELEGRAM_BOT_ALERT` (токен бота), `TELEGRAM_ALERT_CHAT_ID` (ID чатов через запятую; поддерживаются user и group/канал ID).
- Типичные источники алертов:
  - Ошибки планировщика и отправки отчётов (`scheduler.py`).
  - Ошибки обработчиков команд (например, в `asset_router.py`).
  - Проблемы с внешними API курсов валют: при использовании кэша — WARNING, при откате на fallback — ERROR с трейсбеком (`utils/rates.py`).

Пример ручной отправки предупреждения из кода:
```python
import logging
logging.warning("Проверка алёртов: предупреждение")
```
...
### 🔄 CI/CD и автодеплой контейнера
- Настроен процесс непрерывной интеграции и доставки: при пуше в основную ветку собирается Docker‑образ и автоматически раскатывается контейнер на сервер.
- Ожидаемые секреты/переменные в CI:
  - Регистр: `REGISTRY`, `REGISTRY_USERNAME`, `REGISTRY_PASSWORD` (для push образа).
  - Доступ к серверу: `SSH_HOST`, `SSH_USER`, `SSH_PRIVATE_KEY` (или пароль), `DEPLOY_PATH`.
  - Параметры сервиса: `CONTAINER_NAME`, `COMPOSE_FILE` (если используется docker‑compose на сервере).
- Общая схема пайплайна:
  1) Сборка и тегирование образа (например, `latest`/`sha`).
  2) Push образа в реестр.
  3) SSH на сервер и перезапуск контейнера с новым образом (`docker compose pull && docker compose up -d`).

Примечание: конкретная реализация пайплайна может отличаться (GitHub Actions, GitLab CI, Jenkins). Проверьте соответствие ваших секретов и шагов вашему окружению.
